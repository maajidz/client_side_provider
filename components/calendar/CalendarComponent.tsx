...
import {
  UpdateApointmentDateFormSchema,
  updateApointmentDateSchema,
...
const CalendarComponent: React.FC<CalendarComponentProps> = ({
  ...
  const [selectedSlot, setSelectedSlot] = useState<SelectedSlotData | null>(
    null
  );
  ...
  const onSubmit = async (data: UpdateApointmentDateFormSchema) => {
    ...
    try {
      setLoading(true);
      const response = await updateProviderAvailabilityRequest({
        requestData: requestData,
        providerId: "01ab2c92-e75d-48b5-88ff-3c701a0e8fd5",
      });
  ...
  useEffect(() => {
    const events = appointments.flatMap((appointment) =>
      appointment.slots.map((slot) => ({
        id: appointment.id,
        title: `${new Date(`1970-01-01T${slot.startTime}`).toLocaleTimeString(
          [],
  ...
  const handleDeleteClick = async (slotID: string) => {
    ...
    try {
      setLoading(true);
      const response = await deleteProviderAvaialability({
                    ...
                    <FormField
                      ...
                      render={(field) => (
                        <FormItem className="flex flex-col gap-1">
                          ...
                          <Textarea
                            {...field}
                            placeholder="Enter notes for availability"
                          />
                        </FormItem>
                      )}
                    />
