...
const ViewFollowUps = ({ patientDetails }: { patientDetails: UserEncounterData }) => {
    ...
    const fetchAndSetResponse = async () => {
        if (patientDetails.chart?.id) {
            setLoading(true);
            try {
                const response = await getFollowUpData({ chartId: patientDetails.chart?.id });
                if (response) {
    ...
    const handleUpdateFollowUp = async (followUpId: string, updatedData: FollowUpInterface) => {
        setLoading(true);
        try {
            const requestBody = {
    ...
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button variant="ghost" className='text-blue-500 underline' onClick={fetchAndSetResponse}>View Follow-ups</Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-4xl">
                ...
                <div className='flex flex-col gap-5'>
                    ...
                    {prevFollowUps && prevFollowUps.length > 0 ? prevFollowUps.map((followUp, index) => (
                        <div key={index} className='flex justify-between'>
                            ...
                            <Button type="submit" onClick={() => handleUpdateFollowUp(followUp.id, followUp)} className='bg-[#84012A]'>Save changes</Button>
                        </div>
                    )) : (
                        <div>
                            No Follow Ups Found
