...
const StickyNotesDialog = ({
    ...
    const autoSave = async () => {
      ...
      try {
        if (stickyNotesData) {
          ...
          const response = await updateStickyNotesData({
            requestData: requestData,
            id: stickyNotesData.id,
          });
  ...
  }, [content, chartId, stickyNotesData, providerDetails.providerId]);

  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent
        className={`sm:max-w-[425px] bg-[${
          backgroundColor ? backgroundColor : "#F4F39E"
        }]`}
      >
        <DialogHeader>
          <DialogTitle>
            <div className="flex gap-2">
              <Button
                variant={"outline"}
                className="bg-[#F4F39E] border-black"
              ...
              <Button
                variant={"outline"}
                className="bg-[#FFBEFF] border-black"
                onClick={() => setBackgroundColor("#FFBEFF")}
              >
                <div className="h-1"></div>
              </Button>
              <Button
                variant={"outline"}
                className="bg-[#91CFFF] border-black"
                onClick={() => setBackgroundColor("#91CFFF")}
              >
                <div className="h-1"></div>
              </Button>
              <Button
                variant={"outline"}
                className="bg-[#C2CF6C] border-black"
                onClick={() => setBackgroundColor("#C2CF6C")}
              >
                <div className="h-1"></div>
              </Button>
              <Button
                variant={"outline"}
                className="bg-[#FF7878] border-black"
                onClick={() => setBackgroundColor("#FF7878")}
              >
                <div className="h-1"></div>
              </Button>
            </div>
