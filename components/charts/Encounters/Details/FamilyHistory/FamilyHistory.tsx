...
import { deleteFamilyHistory, getFamilyHistoryData } from "@/services/chartDetailsServices";
import { EditFamilyHistoryInterface, FamilyHistoryResponseInterface } from "@/types/familyHistoryInterface";
...
const FamilyHistory = ({ patientDetails }: { patientDetails: UserEncounterData }) => {
  const [loading, setLoading] = useState<boolean>(false);
  const [data, setData] = useState<FamilyHistoryResponseInterface[]>([]);
  const [editData, setEditData] = useState<EditFamilyHistoryInterface | null>(null);
  ...
  const fetchFamilyHistory = useCallback(async () => {
    setLoading(true);
    try {
      const response = await getFamilyHistoryData({ limit: 10, page: 1, userDetailsId: patientDetails.userDetails.id });
      if (response) {
        setData(response);
      }
    } catch (e) {
            ...
            <div>
              {data.map((familyHistory, index) => (
                <div key={index} className='flex flex-col gap-2 border p-2'>
                  <div className='flex justify-between items-center'>
                    <div className='text-base font-semibold'>{familyHistory.id} </div>
                    <div className='flex'>
                      <Button
                        variant={'ghost'}
                        onClick={() => {
                          setEditData({
                            ...
                            activeProblems: familyHistory.activeProblems?.map((problemName) => ({
                      ...
                      <Button
                        variant={'ghost'}
                        onClick={() => handleDeleteFamilyHistory(familyHistory.id)}
                      >
                  ...
                  <div className='flex flex-col gap-1 '>
                    <FormLabels label='Relationship/Age' value={`${familyHistory.relationship}/${familyHistory.age}`} />
                    <FormLabels
